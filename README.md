...
The optional Workforce v1 surface adds `workforce-router`, Engineering/Growth/Research teams, deterministic delivery
and research workflows, a fixed-operation sandbox executor, and a per-user Buzz adapter. See
[`docs/workforce-v1.md`](docs/workforce-v1.md) for its public/private boundary, prompt provenance gate, local profiles,
and production JWT setup.
...
|----------|----------|---------|-------------|
| `OPENAI_API_KEY` | yes | none | OpenAI-compatible key for chat models. |
| `OPENAI_EMBEDDING_MODEL_ID` | no | none | Enables hybrid knowledge retrieval; unset uses PostgreSQL keyword search. |
| `OPENAI_EMBEDDING_API_KEY` | hybrid | none | Dedicated embedding endpoint key. |
| `OPENAI_EMBEDDING_BASE_URL` | hybrid | OpenAI default | Dedicated OpenAI-compatible embedding endpoint. |
