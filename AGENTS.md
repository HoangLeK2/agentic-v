...
Knowledge bases use PostgreSQL keyword search by default, so a chat-only OpenAI-compatible endpoint cannot generate an
empty query vector. Set `OPENAI_EMBEDDING_MODEL_ID` plus a dedicated embedding key/base URL to opt into PgVector hybrid
search. Document contents go into `<table_name>_contents`.
...
|---|---|---|---|
| `OPENAI_API_KEY` | yes | — | OpenAI-compatible key for chat models. |
| `OPENAI_EMBEDDING_MODEL_ID` | no | — | Explicitly enables hybrid knowledge search with this embedding model. Unset uses keyword search. |
| `OPENAI_EMBEDDING_API_KEY` | hybrid | — | Dedicated key for the configured embedding endpoint. |
