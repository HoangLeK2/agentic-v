...
CASES: tuple[Case, ...] = (
    ...
    # Workforce prompt gates. These are release-only because they use role-specific
    # models and judge calls; deterministic provenance coverage lives in tests/.
    Case(
        name="workforce_router_uses_minimal_domain_delegation",
        team=workforce_router,
        input=(
            "Review the authentication code in repository agentos-railway for correctness. "
            "Do not perform market or web research."
        ),
        tags=("release", "workforce"),
        timeout_seconds=180,
        setup=snapshot_learning_state,
        teardown=cleanup_new_learning_state,
        criteria=(
            "Routes the repository task to the Engineering Team only. Does not broadcast to Growth or Research, "
            "does not claim checks ran without executor evidence, and surfaces capability_unavailable when the "
            "workspace executor is not configured."
        ),
    ),
    Case(
        name="engineering_team_keeps_test_and_review_independent",
        team=engineering_team,
        input=(
            "Implement a focused parser fix in repository agentos-railway. Explain the required delivery sequence "
            "without pretending any unavailable operation ran."
        ),
        tags=("release", "workforce"),
        timeout_seconds=180,
        setup=snapshot_learning_state,
        teardown=cleanup_new_learning_state,
        criteria=(
            "Uses the engineering-delivery boundary for mutation, keeps Code separate from Tester and Reviewer, "
            "uses named executor checks rather than raw shell, and reports unavailable capability instead of a "
            "fabricated implementation or test result."
        ),
    ),
    Case(
        name="research_team_requires_claim_level_source_verification",
        team=research_team,
        input="Research whether modular planner-worker systems generalize better across software domains.",
        tags=("release", "workforce", "live"),
        timeout_seconds=240,
        setup=snapshot_learning_state,
        teardown=cleanup_new_learning_state,
        criteria=(
            "Searches iteratively, favors primary sources, has Source Verifier check material claims before final "
            "synthesis, cites direct links near claims, and returns insufficient evidence when support is missing."
        ),
    ),
